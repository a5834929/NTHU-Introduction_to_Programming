...
int main(void)
{
   ...
   int i, j, vk;
   char icon[] = {'G', ' ', '#', '@'};  /* 顯示的字元 */
   int wicon[] = {0xA243, 0xA140, 0xA1B8, 0xA249};
   ...
   while (!IsEnding) {
      ...
      if (!whole_font) {
         ...
         if (status == 3) {
            ...
            for (j = 0; j < WIDTH; j++) {
                if(a[i][j]=='#'){
                putASCII2(2*j + OFFSET_X, i + OFFSET_Y, 0xA1, 13, 0);
                putASCII2(2*j + OFFSET_X+1, i + OFFSET_Y, 0xB8, 13, 0);
                }
                if(a[i][j]=='G'){
                putASCII2(2*j + OFFSET_X, i + OFFSET_Y, 0xA2, 13, 0);
                putASCII2(2*j + OFFSET_X+1, i + OFFSET_Y, 0x43, 13, 0);
                }
                if(a[i][j]=='S'){
                putASCII2(2*j + OFFSET_X, i + OFFSET_Y, 0xA2, 13, 0);
                putASCII2(2*j + OFFSET_X+1, i + OFFSET_Y, 0xE1, 13, 0);
                }
                if(a[i][j]=='E'){
                putASCII2(2*j + OFFSET_X, i + OFFSET_Y, 0xA2, 13, 0);
                putASCII2(2*j + OFFSET_X+1, i + OFFSET_Y, 0xD3, 13, 0);
                }
                if(a[i][j]=='@'){
                putASCII2(2*j + OFFSET_X, i + OFFSET_Y, 0xA1, 13, 0);
                putASCII2(2*j + OFFSET_X+1, i + OFFSET_Y, 0xB9, 13, 0);
                }     
            }
         }
         /* current location */
         if (status == 3) {
            putASCII2(2*sx + OFFSET_X, sy + OFFSET_Y, wicon[status]/256, 9, 0);
            putASCII2(2*sx + OFFSET_X+1, sy + OFFSET_Y, wicon[status]%256, 9, 0);
         } else {
            putASCII2(2*px + OFFSET_X, py + OFFSET_Y, wicon[status]/256, rand()%16, 1);    
            putASCII2(2*px + OFFSET_X+1, py + OFFSET_Y, wicon[status]%256, rand()%16, 1);
         }
            
         /* 在這裡加入你的程式碼 用全形字把地圖內容畫出來
            有一種做法是先產生一個簡單的轉換對照表
            把原本的半形字元 icon[] = {'G', ' ', '#', '@'} ;其中每個字元
            都對應到一組兩個全形碼 例如 wicon[][2] = {0xA2, 0xD5, ...  };  (剩下的請自己填)
            再來只要找到對應的全形碼  分兩次 putASCII2() 畫到螢幕上的相鄰位置就可以  */
        /*  Add your code here to convert half-size fonts to full-size fonts.
            For each full-size character, you need to call putASCII2() twice to display the
            corresonding pair of bytes on screen */
      ...
      putString(OFFSET_X, OFFSET_Y+18, "Use Up, Down, Right, Left keys to change positions    ", 14, 2);
      putString(OFFSET_X, OFFSET_Y+19, "'W': Save the current map in map.txt                  ", 14, 2);
      putString(OFFSET_X, OFFSET_Y+20, "'Ctrl': Switch to full/half font                      ", 14, 2);
     ...
     // int Gx, Gy;
      ...
      if (status == 3) {
        ...
        if(act==1){
         ...
         /*  Add your code here to get all the G's and then move to the exit. */
